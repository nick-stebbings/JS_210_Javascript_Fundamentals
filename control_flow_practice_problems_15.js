...
function indexOf(firstString, secondString) {
  firstCharLoop: for (
    let charIndex = 0;
    charIndex < firstString.length - secondString.length;
    charIndex++
  ) {
...
function lastIndexOf(firstString, secondString) {
  let reverseIndexOf = indexOf(
    [...firstString].reverse().join``,
    [...secondString].reverse().join``
  );
  return reverseIndexOf == -1 ? -1 : firstString.length - reverseIndexOf - secondString.length;
}
