/*
P:
t is quite common to find yourself in a situation where you need to perform a search on some data to find something you're looking for. Imagine that you need to search through the yellow pages to find the phone number of a particular business. A straightforward way to do this would be to go through the yellow pages one business at a time, checking if the current business name is the one you're trying to find.
...
D:
  Input = ary, sorted and object (searchitem)
...
A:
    ...
    If the middle value is greater than 'Pizzeria', do the same as the previous step, but with opposite halves.
   -  SET midpoint
   - COMPARE midpoint value of array with searchitem
     - IF mid value is same, return
   - ELSE IF mid value is greater,  search in  lower half
...
function binarySearch(list, item) {
  ...
  let midObject = list[midpoint];
  switch (true) {
    case list.length == 1 && list[0] != item:
      return -1;
    case item == midObject:
      return midpoint;
    case item < midObject:
      return binarySearch(list.slice(0, midpoint), item);
    case item > midObject:
      let upperSearch = binarySearch(list.slice(midpoint), item);
      return upperSearch == -1 ? -1 : (midpoint + upperSearch);
  }
}
const yellowPages = [
  'Apple Store',
